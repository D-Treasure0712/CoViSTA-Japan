...
async function getData(wave: string) {
  try {
    ...
    if (data.length > 0) {
      ...
      console.log(
        "ユニーク系統数:",
        new Set(data.map((d) => d.lineage.name)).size
      );
      console.log(
        "ユニーク日付数:",
        new Set(data.map((d) => d.date.toISOString().split("T")[0])).size
      );
    } else {
...
export default async function RankPage({
  ...
  const dates = [
    ...new Set<string>(
      data.map(
        (d: CovidDataWithRelations) => d.date.toISOString().split("T")[0]
      )
    ),
  ].sort();
  let lineages = [
    ...new Set<string>(data.map((d: CovidDataWithRelations) => d.lineage.name)),
  ];
    ...
    return (
      <div className="p-8 bg-white">
        ...
        <p className="mb-4 text-amber-700">
          注意:
          CSVファイルのインポート方法に問題があり、系統名が正しく表示されていない可能性があります。
        </p>
        <div className="w-full aspect-[16/9] min-h-[600px] border border-gray-300 rounded-lg bg-white p-4 flex items-center justify-center">
          <div className="text-center">
  ...
  const rankData = dates.map((date) => {
    const dateData = data.filter(
      (d: CovidDataWithRelations) => d.date.toISOString().split("T")[0] === date
    );

    // 各系統の合計比率を計算
    const lineageRatios = lineages.map((lineage) => {
      const lineageData = dateData.filter(
        (d: CovidDataWithRelations) => d.lineage.name === lineage
      );
      const totalRatio = lineageData.reduce(
        (sum: number, d: CovidDataWithRelations) => sum + d.ratio,
        0
      );
    ...
    // 比率で昇順ソートして順位を付ける
    lineageRatios.sort((a, b) => a.totalRatio - b.totalRatio);
    const ranks = new Map(
      lineageRatios.map((item, index) => [item.lineage, index + 1])
    );
  ...
  const plotData = lineages.map((lineage, index) => {
    ...
    console.log(
      `系統 ${lineage} - データポイント数: ${
        yValues.filter((v) => v !== null).length
      }`
    );
  ...
  const layout: Partial<Layout> = {
    ...
    yaxis: {
      ...
      autorange: "reversed",
      tickmode: "array",
      ticktext: Array.from({ length: lineages.length }, (_, i) =>
        String(lineages.length - i)
      ),
      tickvals: Array.from({ length: lineages.length }, (_, i) => i + 1),
      tickfont: {
        color: "#000000",
        size: 12,
      },
    ...
    <div className="p-8 bg-white">
      ...
      <div className="w-full aspect-[16/9] min-h-[600px] border border-gray-300 rounded-lg bg-white p-4">
        ...
        <div style={{ marginTop: "10px", fontSize: "12px", color: "#666" }}>
          取得データ数: {data.length} | 日付数: {dates.length} | 系統数:{" "}
          {lineages.length}
        </div>
      </div>
    </div>
